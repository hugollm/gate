...
class TemplateRendererTestCase(TestCase):
    ...
    def test_can_render_template_from_directory(self):
        ...
        renderer.add_directory('tests/test_template_renderer/site')
    ...
    def test_can_render_templates_from_different_directories(self):
        renderer = TemplateRenderer()
        renderer.add_directory('tests/test_template_renderer/site')
        renderer.add_directory('tests/test_template_renderer/site2')
    ...
    def test_can_render_template_from_package(self):
        renderer = TemplateRenderer()
        renderer.add_package('tests.test_template_renderer.base')
        text = renderer.render('tests.test_template_renderer.base/base.html')
        self.assertEqual(text, '<body></body>')

    def test_can_render_templates_from_different_packages(self):
        renderer = TemplateRenderer()
        renderer.add_package('tests.test_template_renderer.base')
        renderer.add_package('tests.test_template_renderer.hello')
        text = renderer.render('tests.test_template_renderer.hello/hello.html', {'name': 'John'})
